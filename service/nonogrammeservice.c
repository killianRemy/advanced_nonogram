...
unsigned char isNonogramRowCorrectlyFilled(const Nonogram *nonogram, unsigned char index) {
    if (nonogram == NULL) return 0;
    unsigned char width = nonogramGetWidth(nonogram);
    if (width <= index) return 0;

    unsigned char constraintSize = nonogramRowsConstraintsGetSize(nonogram, index);
    unsigned char *constraintArray = nonogramRowsConstraintsToArray(nonogram, index);
    unsigned char constraintArrayIndex = 0;
    ...
    for (unsigned char i = 0; i < width; i++) {
        if (countOn == 0) {
            if (nonogramGetXY(nonogram, i, index) == BLACK) {
                countOn = 1;
                blackCount++;
            }
        } else {
            if (nonogramGetXY(nonogram, i, index) == BLACK) {
                ...
                countOn = 0;
                if (blackCount != constraintArray[constraintArrayIndex]) {
                    return 0;
                } else {
                    blackCount=0;
                    constraintArrayIndex++;
                }
            }
        }
    }
